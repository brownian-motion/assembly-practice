...
section .text
    ...
    ; int argc = *(int*)rsp
    ; rax = argc
    mov rbx, [rsp] ; System V convention, see https://stackoverflow.com/questions/52012290/how-to-get-argc-in-nasm-in-x64

    ; char** env = (char**)(rsp + 8*argc + 16)
    ; rbx = env
    imul rbx, 8
    add rbx, rsp
    add rbx, 16
...
print_argv_iter:
    ...
    ; look for a NULL ptr at the end of the env array
    cmp rsi, 0
    je do_exit
